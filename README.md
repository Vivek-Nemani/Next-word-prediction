...
- Managing memory when tokenizing/padding large tweet corpora—sampling 3k lines keeps RAM manageable.
- GitHub’s 100 MB limit required keeping the dataset out of version control.
- Need to explicitly set the model’s input shape (`model.build((None, max_length-1))`) so `model.summary()` works in Keras 3.
- Hyperparameter tuning and validation splits improve generalization; future work includes more datasets and better decoding strategies.
